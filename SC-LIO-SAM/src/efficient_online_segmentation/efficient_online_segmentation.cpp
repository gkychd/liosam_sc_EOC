...
void EfficientOnlineSegmentation::ResetParameters(const SegmentationParams& params)
{
    params_ = params;
    sectors_.clear();
    sectors_.resize(params_.kNumSectors);//360个sector
    ...
    for (int i=0; i<sectors_.size(); i++) {
        if (i == sectors_.size()-1) {
            sectors_[i].SetReferenceSector(sectors_[0].GetSectorDataPtr());//设置相邻的sector（其为shared_ptr智能指针 指向该sector的16个bin）
            continue;
        }
                                    ...
                                    std::vector<int>* labels_out, bool use_intensity) 
{
    ...
    // step01 register every and each point into corresponding sector! 这部分最耗时
    for (auto& sector : sectors_) {sector.Reset();}
    pcl::transformPointCloud(*cloud_data, *common_cloud_in_base_, sensor_pose_in_base_);//将velodyne系下的点转到base_link系下
    range_image_ = cv::Mat::zeros(params_.kLidarRows, params_.kLidarCols, CV_32FC1);//16 * 1800距离图像初始化都为0（黑色）

    for (std::size_t i=0; i<cloud_data->size(); ++i)
    {
        //计算方位角和俯仰角，去除不符合阈值的点   
        //kLidarVertAngMin 比 kLidarVertFovMin 小0.5度；
        //kLidarVertAngMax 比 kLidarVertAngMax 大0.5度；
        ...
        //rangeImgRowPtr为指向距离图像每个点的指针 这里给距离图像的值赋值（xy维度的距离）
        ...
        //将sectors/bin划分好（360个sector，每个sector有16/32/64个bin），且bin中的z为最小值
