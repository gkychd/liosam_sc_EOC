...
bool SmartLine::TryAddNewBin(const int& newBin, const pcl::PointXYZI& binPoint)
{
    ...
    **/
    /** @brief 原先每个bin的坐标为base_link坐标系下的，且bin的坐标由xyz三维点转化为X(xy距离)，Y(z坐标)的二维坐标
     *  这里将每个bin在base_link坐标系下的X、Y转化到第一个bin点的局部坐标系下，该局部坐标系与base_link坐标系的平移为该bin在世界坐标系下的
     *  X0、Y0，该局部坐标系与base_link坐标系的旋转通过第一个bin和第二个bin在base_link坐标系下的X0、Y0和X1、Y1得到（通过求相对）
     *  以X0、Y0与X1、Y1连线为新的坐标系的X'轴。这样做的目的是：在base_link坐标系下，对于处于平面上的点，其tan(X/Y)接近90度，有奇异性。
     *  在局部坐标系下表示可以避免这种现象。
     * 
     * bin从0-1-2-……一直累加  直到不满足构成水平直线或垂直直线的要求
     * */
    if (bins_.size()==0) {
        start_bin = newBin;
        end_bin = newBin;
        ...
        if (dist > kGroundMaxDistBtwnRings) return false;//8个ring 前面两个点相邻不能超过8个bin
        end_bin = newBin;
        bins_.push_back(newBin);
        slopes_.push_back(0);
        ...
        float curr_k = (curr_point_y - last_point_y) / (curr_point_x - last_point_x);//  相邻的两个bin的 高度差/距离差
        if (std::abs(locTanTheta) > kWallSlopeThres) /*a possible wall line. 角度大于75度*/ {
            for (const auto& existed_slope : slopes_) {
                if (std::abs(existed_slope - curr_k) > kWallSameLineSlopeThres) return false;//10度
            }
        }
        else if (std::abs(locTanTheta) < kGroundSlopeThres) /*a possible ground line. 角度小于10度*/ {
            for (const auto& existed_slope : slopes_) {
                if (std::abs(existed_slope - curr_k) > kGroundSameLineSlopeThres //2度
...
void SmartSector::AddPoint(const pcl::PointXYZI& pointIn, const int& pointIdx, const int& binIdx)
{
    ...
    // 'lowest as indicator' version. 这是bin中的z值取所有在bin中的点的z的最小值
...
void SmartSector::RunLineExtraction()
{
    ...
    for (int i=0; i<sector_data_ptr_->size(); ++i) {
        ...
        else { /* current line ended. 返回false说明直线到这里就中断了，以下提取直线属性并上标签*/
    ...
    if (!ground_bins_.empty()) {
        ...
        对于一个sector中的bin点，在ground_bins中遍历寻找是否是初步地面点，如果是，则参与地面点阈值判断，成功则加入
        到ground_points_indices中*/
...
LineLabel SmartSector::JudgeLineLAbel(SmartLine& smartLine, const std::vector<pcl::PointXYZI>& binVec)
{
    ...
    if (!smartLine.isCalculated) {
        ...
        // smartLine.k_ = deltaY/deltaX;           // strategy A. 第一个bin和最后一个bin表示的斜率
        smartLine.k_ = smartLine.locTanTheta;   // strategy B  第一个bin和第二个bin表示的斜率
    ...
    if ((smartLine.bins_.size()>2
         &&std::abs(smartLine.k_)<kGroundSlopeTolerance //10度
         && std::abs(smartLine.b_+kSensorHeight)<kGroundYInterceptTolerance) //0.5
        || 
        (smartLine.bins_.size()==2 //如果sector中只有两个bin 则阈值的要求更苛刻一些
    ...
    else if (std::abs(smartLine.k_)>kWallSlopeTolerance //75度
            && smartLine.bins_.size()>=kWallLineMinBinNum ) //至少需要3个ring
    {
