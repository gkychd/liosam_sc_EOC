...
public:
    ...
    {
        auto imageTimeStart = ros::Time::now().toSec();
        static double image_time_pre = -1;
        image_time_cur = laserCloudMsg->header.stamp.toSec();
        std::ofstream imageTime("/home/gky/桌面/slam_results/image_lidar_time.csv", std::ios::app);
        imageTime << image_time_cur - image_time_pre << endl;
        image_time_pre = image_time_cur;
        ...
        auto imageTimeEnd = ros::Time::now().toSec();
        ROS_INFO_STREAM("\033[1;32m" << setw(22) << "imageProjection takes" << (imageTimeEnd - imageTimeStart) * 1000 << "ms\033[0m");
        std::ofstream imageTaketime("/home/gky/桌面/slam_results/image_take_time.csv", std::ios::app);
        imageTaketime << (imageTimeEnd - imageTimeStart) * 1000 <<endl;
        resetParameters();
    }
    ...
    void cloudExtraction()
    {
        ...
        for (int i = 0; i < N_SCAN; ++i)
        {
            ...
            for (int j = 0; j < Horizon_SCAN; ++j)
            {
                ...
                {
                    //这个if是根据专门的场景来设置，由于20HK数据集会采集到车身数据，需要将其剔除
                    if(fullCloud->points[j + i*Horizon_SCAN].x >= 0.85 || fullCloud->points[j + i*Horizon_SCAN].x <= -0.85 || fullCloud->points[j + i*Horizon_SCAN].y >= 2.3 || fullCloud->points[j + i*Horizon_SCAN].y <= -1.9){
